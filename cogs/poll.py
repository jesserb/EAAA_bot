...
from constants import CLANICONWHT
...
class PollCog:
    ...
    # Command creates a poll inside an imbeded message, where the options
    # are chosen by selection the corresponding reaction.
    ...
    async def poll(self, ctx, question, *options: str):
        ...
        embed = discord.Embed(title=question, description=begin+''.join(description)+end)
        embed.set_author(name=memb.name+'\n\n', icon_url= CLANICONWHT)
        embed.set_thumbnail(url = CLANICONWHT)
        react_message = await self.bot.say(embed=embed)
        for reaction in reactions[:len(options)]:
            await self.bot.add_reaction(react_message, reaction)
        embed.set_footer(text='Poll ID: {}'.format(react_message.id),icon_url=CLANICONWHT)
        await self.bot.edit_message(react_message, embed=embed)
    
    
    
    # command tallys up the result of a previous poll from the .poll
    # command. Command requires a valid poll ID.
    @commands.command(pass_context=True)
    async def tally(self, ctx, id):
        ...
        try:
            ...
            await self.bot.say(':x: Could not find poll with that ID.')
            return
